{block name='productdetails-details'}
    {* IW: Produktname bereinigt – Suffixe werden im H1 ausgeblendet, URL-Slug bleibt unverändert *}
    {* In JTL liegt der Name HTML-encoded vor (z.B. "Originalzubeh&ouml;r"). *}
    {* Daher müssen alle Replace-Muster ebenfalls die Entity-Schreibweise verwenden. *}
    {assign var='cNameBereinigt' value=$Artikel->cName}
    {* Originalzubehör – HTML-Entity-Varianten *}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Originalzubeh&ouml;r':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' originalzubeh&ouml;r':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Originalzubehoer':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' originalzubehoer':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Original Zubeh&ouml;r':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' original Zubeh&ouml;r':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Original-Zubeh&ouml;r':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Original-Zubehoer':''}
    {* Sicherheitshalber auch die nicht-entity-Varianten, falls Artikel teilweise mit echtem ö gepflegt sind *}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Originalzubehör':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' originalzubehör':''}
    {* Merchandising – alle Schreibvarianten mit Leerzeichen davor *}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Original-Merchandising':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' original-Merchandising':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Original Merchandising':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' original Merchandising':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Orginal-Merchandising':''}
    {assign var='cNameBereinigt' value=$cNameBereinigt|replace:' Orginal Merchandising':''}
    {* Trailing-Trim, falls am Ende noch Leerzeichen oder Bindestriche bleiben *}
    {assign var='cNameBereinigt' value=$cNameBereinigt|trim:" -"}
    ...
    {block name='productdetails-details-form'}
        ...
        {container class="{if $Einstellungen.template.theme.left_sidebar === 'Y' && $boxesLeftActive}container-plus-sidebar{/if}"}
            {form id="buy_form{if !empty($smarty.get.quickView)}-quickview{/if}" action=$Artikel->cURLFull class="jtl-validate"}
                ...
                {assign var='showBrandInHeadline' value=($Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen !== 'N' && !empty($Artikel->cHersteller))}
                {row id="product-offer" class="product-detail"}
                    {block name='productdetails-details-mobile-header'}
                        {col cols=12 class="product-mobile-header d-lg-none"}
                            <div class="product-mobile-header-inner">
                                <div class="product-headline">
                                    <div class="product-title-mobile iw-pdp-h1" role="heading" aria-level="1">
                                        {if $showBrandInHeadline}
                                            <span class="iw-pdp-h1-brand text-muted small">
                                                {if $Einstellungen.artikeldetails.artikel_weitere_artikel_hersteller_anzeigen === 'Y'}
                                                    <a class="text-muted" href="{if !empty($Artikel->cHerstellerHomepage)}{$Artikel->cHerstellerHomepage}{else}{$Artikel->cHerstellerURL}{/if}"{if !empty($smarty.get.quickView)} target="_blank"{/if}>{$Artikel->cHersteller}</a>
                                                {else}
                                                    {$Artikel->cHersteller}
                                                {/if}
                                            </span>
                                        {/if}
                                        <span class="iw-pdp-h1-title">{$cNameBereinigt}</span>
                                    </div>
                                </div>
                                {if ($Artikel->Bewertungen->oBewertungGesamt->nAnzahl > 0) || isset($Artikel->cArtNr)}
                                    <div class="product-meta-row d-flex flex-wrap align-items-center justify-content-between mb-2">
                                        {if ($Einstellungen.bewertung.bewertung_anzeigen === 'Y' && $Artikel->Bewertungen->oBewertungGesamt->nAnzahl > 0)}
                                            <div class="rating-wrapper mb-0">
                                                {if empty($smarty.get.quickView)}
                                                {link href="{$Artikel->cURLFull}#tab-votes"
                                                    id="jump-to-votes-tab-mobile"
                                        ...
                                        {if isset($Artikel->cArtNr)}
                                            <div class="product-meta-sku small text-muted ml-auto text-right">
                                                <span class="product-meta-label">Art.-Nr.:</span>
                                                <span>{$Artikel->cArtNr}</span>
                                            </div>
                                        {/if}
                                    </div>
                                {/if}
                        ...
                        <div class="product-info-inner">
                            <div class="product-headline d-none d-lg-block">
                                {block name='productdetails-details-info-product-title'}
                                    ...
                                    {assign var='showBrandInHeadline' value=($Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen !== 'N' && !empty($Artikel->cHersteller))}
<h1 class="product-title iw-pdp-h1" itemprop="name">
    {if $showBrandInHeadline}
        <span class="iw-pdp-h1-brand text-muted small" itemprop="brand" itemscope itemtype="https://schema.org/Brand">
            {if $Einstellungen.artikeldetails.artikel_weitere_artikel_hersteller_anzeigen === 'Y'}
                <a class="text-muted" href="{if !empty($Artikel->cHerstellerHomepage)}{$Artikel->cHerstellerHomepage}{else}{$Artikel->cHerstellerURL}{/if}" itemprop="url"{if !empty($smarty.get.quickView)} target="_blank"{/if}><span itemprop="name">{$Artikel->cHersteller}</span></a>
            {else}
                <span itemprop="name">{$Artikel->cHersteller}</span>
                {if !empty($Artikel->cHerstellerURL)}<meta itemprop="url" content="{$Artikel->cHerstellerURL}">{/if}
            {/if}
        </span>
    {/if}
    <span class="iw-pdp-h1-title">{$cNameBereinigt}</span>
                            ...
                            {if ($Artikel->Bewertungen->oBewertungGesamt->nAnzahl > 0) || isset($Artikel->cArtNr)}
                                <div class="product-meta-row d-none d-lg-flex flex-wrap align-items-center justify-content-between mb-2">
                                    {if ($Einstellungen.bewertung.bewertung_anzeigen === 'Y' && $Artikel->Bewertungen->oBewertungGesamt->nAnzahl > 0)}
                                        {block name='productdetails-details-info-rating-wrapper'}
                                            <div class="rating-wrapper mb-0" itemprop="aggregateRating" itemscope itemtype="https://schema.org/AggregateRating">
                                ...
                                {block name='productdetails-details-info-essential'}
                                    <ul class="info-essential list-unstyled">
                                        ...
                                        {block name='productdetails-details-info-manufacturer-wrapper'}
                                            {if $Einstellungen.artikeldetails.artikeldetails_hersteller_anzeigen !== 'N'
                                                && isset($Artikel->cHersteller)
                                                && !($showBrandInHeadline|default:false)}
                                                {block name='productdetails-details-include-manufacturer'}
                                ...
                                {row}
                                    {block name='productdetails-details-include-price'}
                                        {if !($Artikel->Preise->fVKNetto == 0 && isset($Artikel->FunktionsAttribute[$smarty.const.FKT_ATTRIBUT_VOUCHER_FLEX]))}
                                            {col}
                                                <div class="iw-price-inline iw-price-inline--detail">
