{block name='layout-header'}
    ...
    <head>
        {* === IW: Title- und Description-Logik für Produktseiten ===
           Berechnet einen optimierten Title (max. 60 Z. mit Suffix, sonst voller Name)
           und überschreibt $meta_title + $meta_description damit alle nachfolgenden
           Stellen (Title-Tag, og:title, Description, og:description) die neuen Werte
           nutzen. Verhindert doppelte Tags durch Drop-Code. *}
        {if isset($Artikel) && !empty($Artikel->cName)}
            {assign var='iwBrand' value=$Artikel->cHersteller|default:''|strip_tags|trim}
            {assign var='iwName'  value=$Artikel->cName|strip_tags|trim}
            {* IW: Suffixe wie "Originalzubehör" / "original Merchandising" aus Title/Description entfernen *}
            {* In JTL liegen Umlaute als HTML-Entity vor (z.B. "Originalzubeh&ouml;r") *}
            {assign var='iwName' value=$iwName|replace:' Originalzubeh&ouml;r':''}
            {assign var='iwName' value=$iwName|replace:' originalzubeh&ouml;r':''}
            {assign var='iwName' value=$iwName|replace:' Originalzubehoer':''}
            {assign var='iwName' value=$iwName|replace:' originalzubehoer':''}
            {assign var='iwName' value=$iwName|replace:' Originalzubehör':''}
            {assign var='iwName' value=$iwName|replace:' originalzubehör':''}
            {assign var='iwName' value=$iwName|replace:' Original Zubeh&ouml;r':''}
            {assign var='iwName' value=$iwName|replace:' Original-Zubeh&ouml;r':''}
            {assign var='iwName' value=$iwName|replace:' Original-Zubehoer':''}
            {assign var='iwName' value=$iwName|replace:' Original-Merchandising':''}
            {assign var='iwName' value=$iwName|replace:' original-Merchandising':''}
            {assign var='iwName' value=$iwName|replace:' Original Merchandising':''}
            {assign var='iwName' value=$iwName|replace:' original Merchandising':''}
            {assign var='iwName' value=$iwName|replace:' Orginal-Merchandising':''}
            {assign var='iwName' value=$iwName|replace:' Orginal Merchandising':''}
            {assign var='iwName' value=$iwName|trim:" -"}
            {if $iwBrand}
                {assign var='iwPattern' value='/^'|cat:$iwBrand|cat:'\s+/i'}
                {assign var='iwNameClean' value=$iwName|regex_replace:$iwPattern:''|trim}
                {assign var='iwFullName' value=$iwBrand|cat:' '|cat:$iwNameClean}
            {else}
                {assign var='iwFullName' value=$iwName}
            {/if}
            {assign var='iwSuffix' value=' | Italo-Welt'}
            {if ($iwFullName|strlen) + ($iwSuffix|strlen) <= 60}
                {assign var='meta_title' value=$iwFullName|cat:$iwSuffix}
            {else}
                {assign var='meta_title' value=$iwFullName}
            {/if}
            {assign var='meta_description' value=$iwFullName|cat:" in unserem Onlineshop &#10004; TOP Preise &#10004; Schneller Versand &#10148; jetzt bestellen!"}
        {/if}

        {* IW: Sonderseiten-SEO (editierbar im JTL Template-Editor) *}
        {* Admin → Design → Template-Editor → NOVAChild → layout → iw_seo_sonderseiten.tpl *}
        {include file='layout/iw_seo_sonderseiten.tpl'}
        ...
        {block name='layout-header-head-meta'}
            ...
            {* IW: Noindex-Logik inkl. NaviFilter *}
            {$noindex = $bNoIndex === true || (isset($Link) && $Link->getNoFollow() === true)}
            {if isset($NaviFilter) && $NaviFilter->getFilterCount() > 0}
                {$noindex = true}
            {/if}
            <meta name="robots" content="{if $noindex}noindex, follow{elseif $robotsContent}{$robotsContent}{else}index, follow{/if}">
        ...
        {block name='layout-header-head-resources'}
            ...
            {* === IW: LCP-Optimierung – Shop-Logo mit höchster Priorität vorladen ===
               Reduziert den "Resource Load Delay" des LCP-Elements. *}
            <link rel="preload" as="image" fetchpriority="high" href="{$ShopLogoURL}">

            {* === IW: LCP-Optimierung – Erstes Produktbild auf Produktseiten vorladen === *}
            {if $nSeitenTyp === $smarty.const.PAGE_ARTIKEL && !empty($Artikel->Bilder)}
                {assign var="lcpMini"       value=$Artikel->Bilder[0]->cURLMini}
                {assign var="lcpKlein"      value=$Artikel->Bilder[0]->cURLKlein}
                {assign var="lcpNormal"     value=$Artikel->Bilder[0]->cURLNormal}
                {assign var="lcpGross"      value=$Artikel->Bilder[0]->cURLGross}
                {assign var="lcpWebpMini"   value=$lcpMini|replace:".jpeg":".webp"|replace:".jpg":".webp"|replace:".png":".webp"}
                {assign var="lcpWebpKlein"  value=$lcpKlein|replace:".jpeg":".webp"|replace:".jpg":".webp"|replace:".png":".webp"}
                {assign var="lcpWebpNormal" value=$lcpNormal|replace:".jpeg":".webp"|replace:".jpg":".webp"|replace:".png":".webp"}
                {assign var="lcpWebpGross"  value=$lcpGross|replace:".jpeg":".webp"|replace:".jpg":".webp"|replace:".png":".webp"}
                <link rel="preload" as="image" fetchpriority="high"
                      href="{$lcpWebpGross}"
                      imagesrcset="{$lcpWebpMini} 120w, {$lcpWebpKlein} 640w, {$lcpWebpNormal} 900w, {$lcpWebpGross} 1600w"
                      imagesizes="(min-width: 992px) 380px, 100vw"
                      type="image/webp">
            {/if}
        ...
        {* Shopauskunft Widget Loader (einmalig) *}
        <script src="https://widget.shopauskunft.de/assets/widget.js" defer></script>
