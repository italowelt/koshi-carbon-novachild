    ...
    {block name='checkout-inc-order-items-order-items'}
        {block name='checkout-inc-order-items-order-items-header'}
            {row class="checkout-items-header text-accent d-none d-lg-flex"}
                ...
                {col cols=1 class="text-center-util"}{lang key="vat" section="productDetails"}{/col}
                {if $Einstellungen.kaufabwicklung.bestellvorgang_einzelpreise_anzeigen === 'Y'}
        ...
        {foreach JTL\Session\Frontend::getCart()->PositionenArr as $oPosition}
            {if !$oPosition->istKonfigKind()}
                {row class="type-{$oPosition->nPosTyp} checkout-items-item{if $tplscope === 'confirmation'} iw-checkout-confirmation-item{if $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_ARTIKEL || $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_GRATISGESCHENK} iw-confirmation-item-product{else} iw-confirmation-item-service{/if}{/if}"}
                    {if $tplscope === 'confirmation'
                        ...
                        <div class="iw-confirmation-service-mobile d-lg-none">
                            <span class="iw-confirmation-service-mobile__name">
                                {$oPosition->cName|transByISO}{if isset($oPosition->discountForArticle)}{$oPosition->discountForArticle|transByISO}{/if}
                            </span>
                            <strong class="iw-confirmation-service-mobile__price">
                    ...
                    {block name='checkout-inc-order-items-items-main-content'}
                        {col cols=$cols lg=$itemInfoCols class="checkout-items-item-main"}
                            ...
                            || $oPosition->nPosTyp === $smarty.const.C_WARENKORBPOS_TYP_GRATISGESCHENK}
                                {block name='checkout-inc-order-items-product-data-link'}
                                    <p>{link href=$oPosition->Artikel->cURLFull title=$oPosition->cName|transByISO|escape:'html'}{$oPosition->cName|transByISO}{/link}</p>
                                {/block}
                        ...
                        {block name='checkout-inc-order-items-vat'}
                            {col cols=$cols lg=1 class="checkout-items-item-vat text-nowrap-util"}
                                <span class="checkout-items-item-title">{lang key="vat" section="productDetails"}:</span> {$oPosition->fMwSt}{if empty($oPosition->fMwSt) === false}%{/if}
                            {/col}
                        {/block}
